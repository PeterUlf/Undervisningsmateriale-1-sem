...
sidebar:
  label: "Astro – Deploy med GitHub og Netlify"
  order: 7
  badge:
    text: Tema 7
    variant: tip
...
<Slide>
## Video: Deploy med Netlify

<YouTubeVideo id="kieD3VxGDww" />
...
1. Gå til dit site → **Project configuration → Environment variables**
2. Tilføj de samme nøgler som i din lokale `.env` (fx `SUPABASE_URL`, `SUPABASE_PUBLISHABLE_KEY`)
3. Hvis du ser scope-indstillinger, så sørg for at **Builds** er valgt
...
1. [Netlify + Supabase: .env og auto-build →](/astro/netlify-supabase-env-og-auto-build/)
2. [Gå til Figma →](/astro/figma/)
</Slide>
