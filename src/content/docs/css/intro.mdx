...
Man sætter det typisk på alle elementer og deres pseudo-elementer i starten af sin CSS-fil:
...
- **`em`** – relativ til elementets font-størrelse; på `font-size` er den relativ til forælderen
- **`%`** – relativ til noget andet, som afhænger af den property du bruger
- **`vw` / `vh`** – procent af viewportens bredde/højde
- **`dvw` / `dvh`** – procent af den synlige viewport, som kan ændre sig på mobil
