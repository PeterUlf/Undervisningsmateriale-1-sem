...
3. <code>params.get("<span class="param-key-red">id</span>")</code> finder værdien til `id`
...
## Øvelse: læs `category` fra URL og vis den i HTML

Lav en ny mappe på din computer, fx `url-parametre-oevelse`, og åbn mappen i VS Code.
...
<html lang="da">
  <head>
    ...
    <title>URL-parametre øvelse</title>
  </head>
  <body>
    <h1>URL-parametre øvelse</h1>
    <p id="resultat">Venter på kategori...</p>
...
Sæt en **tom JavaScript-fil** i `index.js`, og byg løsningen selv ud fra disse trin:

```javascript
// 1) Læs query-delen fra URL'en (delen efter ?)
// 2) Lav et URLSearchParams-objekt
// 3) Hent værdien for "category"
// 4) Find <p id="resultat"> i HTML
// 5) Vis enten category-værdien eller en besked hvis den mangler
```

### Din opgave

1. Start siden i browseren.
2. Test først uden parameter: `index.html`
3. Test derefter med parameter: `index.html?category=rumkatte`
4. Få siden til at vise værdien fra URL'en på HTML-siden (ikke kun i `console.log`).

Hint:

- Brug `window.location.search`
- Brug `new URLSearchParams(...)`
- Brug `params.get("category")`
- Tænk over hvad der skal ske, når `category` er `null`
...
## Øvelse: flere sjove URL-parametre

Denne øvelse minder teknisk om den forrige: læs parametre fra URL'en og vis dem i HTML.

Lav en ny mappe, fx `url-parametre-oevelse-2`, og åbn den i VS Code.
...
<html lang="da">
  <head>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <title>Sjove URL-parametre</title>
  </head>
  <body>
    <h1>Min skøre profil fra URL</h1>
    <p id="resultat">Venter på parametre...</p>

    <ul>
      <li>Kategori: <span id="categoryValue">-</span></li>
      <li>Superkraft: <span id="superkraftValue">-</span></li>
      <li>Snack: <span id="snackValue">-</span></li>
    </ul>
...
```javascript
// 1) Læs query-delen fra URL'en
// 2) Lav et URLSearchParams-objekt
// 3) Hent "category", "superkraft" og "snack" med params.get(...)
// 4) Find spans i HTML med querySelector(...)
// 5) Vis værdierne i de rigtige spans
// 6) Hvis en værdi mangler, vis fx "ukendt"
```

### Din opgave

1. Start siden i browseren.
2. Test uden parametre: `index.html`
3. Test med parametre: `index.html?category=rumkatte&superkraft=usynlighed&snack=nachos`
4. Vis alle tre parametre i hver deres felt på siden.
5. Lav en samlet tekst i `#resultat`, fx en sjov sætning bygget fra parametrene.

Hint:

- Brug `new URLSearchParams(window.location.search)`
- Brug `params.get("category")`
- Brug `params.get("superkraft")`
- Brug `params.get("snack")`
- Husk at en manglende parameter giver `null`
...
## Bonus: link til næste 10 produkter via URL-parametre

Idé:

1. Send `start` og `limit` i linket (fx `?start=10&limit=10`)
2. Modtag dem på siden med `URLSearchParams`
3. Brug dem i fetch-URL'en

Eksempel på URL:

`productlist.html?category=Footwear&start=10&limit=10`

### Send parametre i "Næste 10"-link
...
const nextHref = category
  ? `productlist.html?category=${encodeURIComponent(category)}&start=${nextStart}&limit=${limit}`
  : `productlist.html?start=${nextStart}&limit=${limit}`;

document.querySelector("#nextLink").href = nextHref;
```

### Modtag parametre og brug dem i API-kaldet
...
let fetchUrl = `https://kea-alt-del.dk/t7/api/products?start=${start}&limit=${limit}`;
...
Simpel HTML til knappen:

```html
<a id="nextLink" href="#">Næste 10 produkter</a>
```
